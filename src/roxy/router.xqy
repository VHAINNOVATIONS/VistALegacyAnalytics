...
declare variable $DO-PROFILE := xdmp:get-request-field('_roxy_profile') ;

(: Handle any profiler results. There may be more than one!
 : Today, this simply saves the results to a server-local file.
 : Future enhancements might extend this to store the results in a database,
 : return as part of the page, etc.
 :)
declare function router:profile-finish()
{
  xdmp:save(
    fn:concat('/tmp/', fn:current-dateTime(), '-', xdmp:request(), '.xml'),
    element cprof:reports { cprof:report() })
};

(: Entry point for query-route and update-route main modules.
 : All roads start here.
 : TODO enable cprof and handle any results.
 :)
declare function router:route()
{
  (: Conditionally enable profiling. :)
  if (fn:not($DO-PROFILE)) then () else cprof:enable(),
  ...
  (: Finish any profiling. :)
  , if (fn:not($DO-PROFILE)) then () else router:profile-finish()
};