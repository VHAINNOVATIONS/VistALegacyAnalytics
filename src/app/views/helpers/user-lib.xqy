...
as xs:boolean
{
  xdmp:has-privilege('ns://va.gov/2012/ip401/logged-in', 'execute')
};
...
declare function uv:login-form($redirect-to as xs:string?)
as element()
{
  <form id="login" action="{$LOGIN-PATH}" method="POST">
  {
    form:text-input("Username:", "username", "username"),
    form:password-input("Password:", "password", "password"),
    <input type="hidden" name="redirect-to" value="{ ($redirect-to, '/')[1] }"/>
  }
  ...
  <script language="JavaScript" type="text/javascript">
    <!-- hide script from old browsers
      $(document).ready(function() {{
        ...
        $('form#login input[name="username"]')[0].focus();
      }});
      // end hiding contents -->
  </script>
  </form>
