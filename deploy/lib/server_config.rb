...
class ServerConfig < MLClient
  ...
  def corb
    ...
    cp = File.expand_path("../java/corb.jar", __FILE__)
    cp << path_separator
    cp << xcc_file
    ...
    runme = %Q{java -cp "#{cp}" com.marklogic.developer.corb.Manager #{connection_string} #{collection_name} #{xquery_module} #{thread_count} #{uris_module} #{module_root} #{modules_database} #{install}}
    logger.info runme
    `#{runme}`
  end
      ...
      if @properties['ml.test-modules-db'].present? &&
        ...
        %Q{
          <http-server>
            ...
            <modules name="@ml.modules-db"/>
            <root>@ml.modules-root</root>
            <authentication>@ml.test-authentication-method</authentication>
            <default-user name="@ml.default-user"/>
            <url-rewriter>@ml.url-rewriter</url-rewriter>
            <error-handler>@ml.error-handler</error-handler>
          </http-server>
